...
describe(`real PostgreSQL terminal row retention (${mode})`, () => {
  ...
  it('applies immutable migrations through 0009 for fresh and 0001-0008 upgrade databases', async () => {
    expect(migratedVersions).toEqual(mode === 'legacy-upgrade' ? [9] : [1, 2, 3, 4, 5, 6, 7, 8, 9]);
    const ledger = await database.query<{ version: string }>('SELECT version FROM schema_migrations ORDER BY version');
    expect(ledger.rows.map((row) => Number(row.version))).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9]);
