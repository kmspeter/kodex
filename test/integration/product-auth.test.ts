...
describe('PostgreSQL product authentication API', () => {
  ...
  beforeAll(async () => {
    ...
    if (!ledger.rows[0].present) {
      ...
      if (upgraded.map((migration) => migration.version).join(',') !== '6,7,8,9,10,11,12,13') {
        throw new Error('Auth integration did not apply migrations 0006-0013 during upgrade');
      }
    } else {
