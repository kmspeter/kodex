...
if (!npmCli) throw new Error('Run this PostgreSQL harness through npm run test:abuse-rate-limit-postgres.');
...
for (const mode of ['fresh', 'legacy-upgrade']) {
  ...
  try {
    postgres = await startIsolatedPostgres({
      database: `kodex_abuse_limit_${mode.replace('-', '_')}_test`,
      namePrefix: `kodex-abuse-limit-${mode}-test`,
    });
    await run(process.execPath, [vitestCli, 'run', '--config', 'vitest.abuse-rate-limit-postgres.config.ts'], {
      cwd: repositoryRoot,
      env: {
        ...process.env,
        ...
        ABUSE_RATE_LIMIT_MIGRATION_MODE: mode,
      },
    });
  } finally {
    await postgres?.stop();
