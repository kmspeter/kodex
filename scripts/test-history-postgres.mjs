...
if (!npmCli) throw new Error('Run this harness through npm run test:history-postgres.');
const vitestCli = path.join(repositoryRoot, 'node_modules', 'vitest', 'vitest.mjs');
let postgres;

try {
  postgres = await startIsolatedPostgres({
    database: 'kodex_history_test',
    namePrefix: 'kodex-history-test',
  });
  const env = {
    ...process.env,
  ...
  await run(
    process.execPath,
    [vitestCli, 'run', '--config', 'vitest.history-postgres.config.ts'],
