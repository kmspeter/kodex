...
if (!npmCli) throw new Error('Run this harness through npm run test:email-verification-postgres.');
...
for (const mode of ['fresh', 'legacy-upgrade']) {
  ...
  try {
    postgres = await startIsolatedPostgres({
      database: `kodex_email_verification_${mode.replace('-', '_')}`,
      namePrefix: `kodex-email-verification-${mode}`,
    });
    await run(process.execPath, [vitestCli, 'run', '--config', 'vitest.email-verification.config.ts'], {
      cwd: repositoryRoot,
      env: {
        ...process.env,
        KODEX_DISABLE_ENV_FILE: '1',
        EMAIL_VERIFICATION_DATABASE_URL: postgres.databaseUrl,
        EMAIL_VERIFICATION_MIGRATION_MODE: mode,
      },
    });
  } finally {
    await postgres?.stop();
