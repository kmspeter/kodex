...
const SECRET_RULES = [
  ...
  },
  {
    ...
    expression: /(?:^|["'])(?:AUTH_COOKIE_SECRET|AUTH_PASSWORD_RESET_DELIVERY_BEARER_TOKEN|DATABASE_URL|KODEX_LOCAL_LLM_API_KEY|KODEX_OPERATIONS_BEARER_TOKEN|OPENAI_API_KEY|PRODUCT_DB_(?:ADMIN|APP|MIGRATION)_PASSWORD|PRODUCT_DB_MIGRATION_URL|PRODUCT_OPERATIONS_BEARER_TOKEN)["']?\s*[:=]\s*["']?([^\s,"'`;}{]{20,})/giu,
    candidate: (match) => match[1],
  },
];
