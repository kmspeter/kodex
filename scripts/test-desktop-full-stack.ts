...
import { startResponsesLoopbackFixture } from '../test/fixtures/responses-loopback';
import { startIsolatedPostgres } from './lib/isolated-postgres.mjs';

const repositoryRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const npmCli = process.env.npm_execpath;
if (!npmCli) throw new Error('Run this acceptance harness through npm run test:desktop-full-stack.');
...
let fixture: Awaited<ReturnType<typeof startResponsesLoopbackFixture>> | undefined;
let cleaning = false;
...
try {
  ...
  postgres = await startIsolatedPostgres({
    database: 'kodex_desktop_ui_test',
    namePrefix: 'kodex-desktop-ui-test',
  }) as IsolatedPostgres;
  fixture = await startResponsesLoopbackFixture();
  const env: NodeJS.ProcessEnv = {
    ...
    KODEX_DESKTOP_ACCEPTANCE_DISPLAY_NAME: 'Desktop Acceptance User',
    KODEX_DESKTOP_ACCEPTANCE_EMAIL: `desktop-ui-${randomUUID()}@example.invalid`,
    KODEX_DESKTOP_ACCEPTANCE_PASSWORD: 'desktop acceptance password',
  ...
  await run(
    electronBinary,
    [path.join(repositoryRoot, 'apps', 'desktop', 'main.mjs'), '--full-stack-acceptance'],
    { cwd: repositoryRoot, env },
    210_000,
    'Desktop renderer full-stack acceptance',
  );

  if (fixture.requests.length < 2) throw new Error('The loopback Responses fixture did not receive the complete tool round trip.');
  if (fixture.requests.some((request) => request.headers.authorization !== undefined)) {
    throw new Error('The loopback Responses fixture received an unexpected Authorization header.');
  }
  if (!fixture.sawToolOutput() || !fixture.toolOutputContainsExpectedText() || !fixture.toolOutputSucceeded()) {
    throw new Error('The loopback Responses fixture did not observe the fixed successful echo tool output.');
  }
  process.stdout.write('Desktop full-stack acceptance passed: DOM auth -> settings -> agent/tool -> Saved DB History -> logout.\n');
} catch (error) {
  ...
  if (artifactDirectory && existsSync(artifactDirectory)) {
    const retainedArtifacts = path.join(os.tmpdir(), `kodex-desktop-ui-failure-${randomUUID()}`);
    try {
      await rename(artifactDirectory, retainedArtifacts);
      process.stderr.write(`Desktop acceptance artifacts retained under ${retainedArtifacts}.\n`);
    } catch {
      process.stderr.write('Desktop acceptance artifacts could not be retained safely; runtime data will still be removed.\n');
    }
  }
  throw error;
} finally {
