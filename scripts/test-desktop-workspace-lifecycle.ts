...
if (!npmCli) throw new Error('Run this acceptance harness through npm run test:desktop-workspace-lifecycle.');
...
try {
  ...
  postgres = await startIsolatedPostgres({
    database: 'kodex_desktop_workspace_lifecycle_test',
    namePrefix: 'kodex-desktop-workspace-lifecycle-test',
  }) as IsolatedPostgres;
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    KODEX_DESKTOP_ACCEPTANCE_DISPLAY_NAME: 'Desktop Lifecycle Owner',
    KODEX_DESKTOP_ACCEPTANCE_EMAIL: `desktop-lifecycle-${randomUUID()}@example.invalid`,
    KODEX_DESKTOP_ACCEPTANCE_PASSWORD: 'desktop lifecycle acceptance password',
    KODEX_DESKTOP_ACCEPTANCE_RENAMED_WORKSPACE_NAME: '수명주기 팀 Ω',
    KODEX_DESKTOP_ACCEPTANCE_TARGET_WORKSPACE_NAME: 'Lifecycle Archive Target',
  ...
  await run(
    electronBinary,
    [path.join(repositoryRoot, 'apps', 'desktop', 'main.mjs'), '--workspace-lifecycle-acceptance'],
    { cwd: repositoryRoot, env },
    240_000,
    'Desktop workspace lifecycle acceptance',
  );
  process.stdout.write('Desktop workspace lifecycle acceptance passed: DOM create -> Unicode rename -> exact-confirmation soft archive -> fallback runtime -> denied archived scope.\n');
} catch (error) {
  ...
  if (artifactDirectory && existsSync(artifactDirectory)) {
    const retainedArtifacts = path.join(os.tmpdir(), `kodex-desktop-workspace-lifecycle-failure-${randomUUID()}`);
    try {
      await rename(artifactDirectory, retainedArtifacts);
      process.stderr.write(`Desktop workspace lifecycle acceptance artifacts retained under ${retainedArtifacts}.\n`);
    } catch {
      process.stderr.write('Desktop workspace lifecycle acceptance artifacts could not be retained safely; runtime data will still be removed.\n');
    }
  }
  throw error;
} finally {
