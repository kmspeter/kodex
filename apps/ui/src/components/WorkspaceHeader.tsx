...
export function WorkspaceHeader(props: {
  ...
  return <header className="workspace-header"><div className="title-group">
    ...
    <button className={`toolbar-button change-button ${props.detailsOpen ? 'is-active' : ''}`} onClick={() => { props.onMenu(null); props.onToggleDetails(); }}><GitCommitHorizontal size={14} /><span>Review changes</span><span className="change-count plus">+{props.git.totals.added}</span><span className="change-count minus">−{props.git.totals.removed}</span></button>
