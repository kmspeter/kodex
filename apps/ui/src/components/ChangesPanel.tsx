...
export function ChangesPanel(props: { git: GitStatus; selectedFile: string; diff: string; onSelect: (path: string) => void; onClose: () => void; onRefresh: () => void }) {
  return <aside className="detail-panel" aria-label="Workspace changes"><div className="detail-header"><div><strong>Changes</strong><span>{props.git.isRepository ? `${props.git.branch} · staged / unstaged` : 'Not a Git repository'}</span></div><div className="detail-actions"><button className="icon-button" aria-label="Refresh changes" onClick={props.onRefresh}><RefreshCw size={14} /></button><button className="icon-button" aria-label="Close changes panel" onClick={props.onClose}><X size={15} /></button></div></div>
