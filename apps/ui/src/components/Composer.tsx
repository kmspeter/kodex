...
export function Composer(props: {
  ...
  return <div className="composer-wrap"><div className="composer">
    ...
    <div className="composer-footer"><div className="composer-left"><span className="composer-select"><ShieldCheck size={13} /> {props.settings.sandbox} · shell network {props.settings.network.shell ? 'on' : 'off'}</span><span className="composer-select branch-select"><GitBranch size={13} /> {props.git.branch || 'not a Git repository'}</span></div><div className="composer-right">
      <div className="menu-anchor"><button className="model-select" disabled={!props.models.length} aria-expanded={props.modelOpen} onClick={() => props.onModelOpen(!props.modelOpen)}>{current?.displayName || 'App Server default'} <span>{props.effort}</span> <ChevronDown size={11} /></button>{props.modelOpen && <div className="popover composer-popover model-popover">{props.models.map((entry) => <button key={entry.id} onClick={() => { props.onModel(entry.model, entry.defaultReasoningEffort); props.onModelOpen(false); }}><WandSparkles size={14} /> {entry.displayName} {entry.model === props.model && <Check size={13} />}</button>)}</div>}</div>
