...
function SettingsDialog(props: Parameters<typeof Dialogs>[0]) {
  ...
  return <div className="settings-layout"><nav className="settings-nav">{tabs.map((name) => <button className={tab === name ? 'is-selected' : ''} key={name} onClick={() => setTab(name)}>{name}</button>)}</nav><div className="settings-content"><div className="settings-page-title"><h3>{tab}</h3><p>Kodex local host settings</p></div>
    ...
    {tab === 'Agent' && <><div className="setting-row"><div><strong>Codex binary</strong><span>{props.bootstrap.engine.binary ?? 'Run npm run codex:build'}</span></div><span className="status-pill">{props.bootstrap.engine.binarySource ?? 'missing'}</span></div><div className="setting-row"><div><strong>Official config</strong><span>{props.codexConfig ? `${props.codexConfig.layers?.length ?? 0} layers read for this workspace` : 'Unavailable until App Server is connected'}</span></div><span className="status-pill">App Server v2</span></div><div className="setting-row"><div><strong>Sandbox</strong><span>Applied through official thread/turn parameters.</span></div><select value={settings.sandbox} onChange={(event) => toggle({ sandbox: event.target.value as KodexSettings['sandbox'] })}><option value="read-only">read-only</option><option value="workspace-write">workspace-write</option><option value="danger-full-access">danger-full-access</option></select></div><div className="setting-row"><div><strong>Approval policy</strong><span>Official App Server approval requests are shown in the conversation.</span></div><select value={settings.approvalPolicy} onChange={(event) => toggle({ approvalPolicy: event.target.value as KodexSettings['approvalPolicy'] })}><option value="untrusted">untrusted</option><option value="on-request">on-request</option><option value="never">never</option></select></div></>}
